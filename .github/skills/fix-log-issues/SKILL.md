...
1. Verify no other docs need updating by searching for related references.

### `markdown-lint`

One or more markdown formatting issues are present in the file at `Path`.

**Fix procedure:**

1. Open the file at `Path` and locate the violation described in the
   `Description` field.
1. Edit the file manually to resolve the violation (e.g., fix heading
   levels, add blank lines around code fences, correct list formatting).
1. Review the full file for any adjacent violations of the same rule.
...
A JavaScript syntax or validation error was reported.
