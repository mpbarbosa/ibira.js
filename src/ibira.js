...
export class IbiraAPIFetcher {
	...
	async fetchDataPure(currentCacheState, currentTime = Date.now(), networkProvider = null) {
		...
		try {
			...
			const networkFn =
				networkProvider || (() => this._performSingleRequest(new AbortController()));
...
export class IbiraAPIFetchManager {
	...
	setRetryConfig(retryConfig = {}) {
		...
		if (retryConfig.retryMultiplier !== undefined)
	...
	setRetryConfigForUrl(url, retryConfig = {}) {
		...
		if (this.fetchers.has(url)) {
			...
			const newOptions = {
				...
				maxRetries:
					retryConfig.maxRetries !== undefined ? retryConfig.maxRetries : oldFetcher.maxRetries,
				retryDelay:
					retryConfig.retryDelay !== undefined ? retryConfig.retryDelay : oldFetcher.retryDelay,
				retryMultiplier:
					retryConfig.retryMultiplier !== undefined
						? retryConfig.retryMultiplier
						: oldFetcher.retryMultiplier,
